...
* [`src/wg_link/web/`](src/wg_link/web/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/wg_link/web/core.cljs): contains the SPA entry point, `init`
...
5. Clone this repo and open a terminal in the `wg-link.web` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/wg_link/web/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns wg-link.web.example
  (:require [wg-link.web.config :as config])
